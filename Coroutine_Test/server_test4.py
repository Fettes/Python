...
async def main(args):
    ...
    server = loop.create_server(ServerProtocol(game), 'localhost', 1024)
    task_list = [asyncio.ensure_future(a) for a in game.agents]
    ...
    c1 = mygen(strlist)
    c2 = mygen(intlist)
    task_list.append(asyncio.ensure_future(c1))
    task_list.append(asyncio.ensure_future(c2))
